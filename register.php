...
     <?php
     ...
     if(isset($_POST['submit'])) {
		...
		$ln = strtolower(stripslashes($_POST['lname']));
		$fn = strtolower(stripslashes($_POST['fname']));
        ...
        if($pass == $confpass) {
            if(strlen($pass) >= 5) 
            {
					...
					{
						$userid = mysql_num_rows(mysql_query('select userid from users_info where email="'.$em.'"'));
						if($userid == 0) 
						{
							$numid = mysql_num_rows(mysql_query('select userid from users'));
							$id = $numid+1;
							echo 'the new userid'.$id;
							//&& mysql_query('INSERT INTO users_info(userid, email, dob) VALUES ("'.$id.'","'.$em.'","'.$dob.'")')
							if(mysql_query('insert into users(userid, fname, lname, password) values ("'.$id.'","'.$fn.'", "'.$ln.'","'.$password.'")') && mysql_query('INSERT INTO users_info(userid, email, dob) VALUES ("'.$id.'","'.$em.'","'.$dob.'")'))
							{   
								?>
...
<div class="container">
	...
	<fieldset>
		...
		<label class="left" for="email"><strong>Date of Birth: </strong></label>
		<label>
			<select name="month">
		<option></option>
